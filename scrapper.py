                ...
                for container in flexologist_work_day:
                    ...
                    if len(children) > 0:
                        for child in children:
                            infoDict={}
                            first_timer = child.query_selector("[title='first time visitor']")
                            if first_timer:
                            ...
                            try:
                                ...
                                if modal_details:
                                    ...
                                    result = handle_scraping_analysis("what does this entail?. Comment on areas of opportunity based on information provided, technical quality, prescribed actions, etc. Compile a quality score based on rubric. please return output in well formatted json {'client_name', 'member_rep_name', 'flexologist_name', 'analysis', 'rubric_analysis', 'status', 'booking_date'}. Note: always return an output" , modal_details.inner_html())
                                  
                                    infoDict['location'] = location_name
                                    infoDict['data'] = result['content'] if 'content' in result and result['content'] else result
